...
In this spec's folder, create this file: `.design-systems/specs/[this-spec]/orchestration.yml`.
...
Loop through each task group in `.design-systems/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned subagent specified in `orchestration.yml`.

For each delegation, provide the subagent with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `.design-systems/specs/[this-spec]/spec.md`
- Instruct subagent to:
  - Perform their implementation
  - Check off the task and sub-task(s) in `.design-systems/specs/[this-spec]/tasks.md`
...
3. Compile the list of file references to those standards, one file reference per line, using this logic for determining which files to include:
   ...
   b. If the item under standards ends with "*" then it means that all files within this folder or sub-folder should be included. For example, `frontend/*` means include all files and sub-folders and their files located inside of `.design-systems/standards/frontend/`.
   c. If a file ends in `.md` then it means this is one specific file you must include in your list of files. For example `backend/api.md` means you must include the file located at `.design-systems/standards/backend/api.md`.
   d. De-duplicate files in your list of file references.

##### Output Format

All the backeend related artifacts/code should be inside the folder "backend" and the frontend related code/artifact should be inside the folder "frontend" .The compiled list of standards should look something like this, where each file reference is on its own line and begins with `@`. The exact list of files will vary:

```
@.design-systems/standards/global/coding-style.md
@.design-systems/standards/global/conventions.md
@.design-systems/standards/global/tech-stack.md
@.design-systems/standards/backend/api/authentication.md
@.design-systems/standards/backend/api/endpoints.md
@.design-systems/standards/backend/api/responses.md
@.design-systems/standards/frontend/css.md
@.design-systems/standards/frontend/responsive.md
```
